...
///////////////////////////////////////////////////////////////////////////
// When editing here, probably also need to edit get-valid-membership-for-livestream
///////////////////////////////////////////////////////////////////////////

module.exports = {
  friendlyName: 'Populate Class.user_has_access_to_livestream',
  ...
  fn: async function (inputs, exits) {
    ...
    // Already populated??
    if (typeof inputs.classes[0].user_has_access_to_livestream !== 'undefined') {
      return exits.success(inputs.classes);
    }

    _.each(inputs.classes, cls => {
      cls.user_has_access_to_livestream = false;
    });
    ...
    const earliestClassDate = moment(classesSortedByDate[0].date).tz('Europe/Copenhagen');

    const latestClassDate = moment(classesSortedByDate[classesSortedByDate.length - 1].date).tz('Europe/Copenhagen');
    ...
    /*if (_.find(_.concat(membershipTypes, classPassTypes), accessItem => accessItem.has_max_number_of_simultaneous_bookings)) {
      futureSignups = await sails.helpers.classes.getFutureSignups(userId);
    }*/
    ...
    _.each(inputs.classes, classItem => {
      ...
      _.each(memberships, membership => {
        ...
        const membershipClassTypeIds = _.map(membership.membership_type.class_types_livestream, 'id');
        ...
        switch (membership.status) {
          ...
          case 'active':
            classItem.user_has_access_to_livestream = true;
            return false;
      ...
      if (classItem.user_has_access_to_livestream) {
        return;
      }
      ...
      _.each(classPasses, classPass => {
        ...
        const classPassClassTypeIds = _.map(classPass.class_pass_type.class_types_livestream, 'id');

        if (!_.includes(classPassClassTypeIds, classItem.class_type_id)) return;
