...
///////////////////////////////////////////////////////////////////////////
// When editing here, probably also need to edit user-has-access-to-class
///////////////////////////////////////////////////////////////////////////

module.exports = {

  friendlyName: 'Get valid memberships for class',

  description: 'Gets all valid memberships that a specific user has for a class, excluding the ones already used for the class.',
  ...
  fn: async (inputs, exits) => {
    ...
    let memberships = await MembershipObjectionObj.query().from({m: 'membership'})
      ...
      .andWhere(function () {
        this.where('status', 'active')
          .orWhere(qb => {
            qb.where('status', 'cancelled_running')
    ...
    memberships = _.filter(
      memberships,
      membership => {
        let membershipTypeClassTypeIds = _.map(membership.membership_type.class_types, 'id');
        return _.includes(membershipTypeClassTypeIds, classItem.class_type);
      },
    );

    ///////////////////////
    // Membership already used for class?
    ///////////////////////
    const existingSignupsForClass = await ClassSignup.find({
      archived: false,
      cancelled_at: 0,
      user: userId,
      class: classItem.id,
    ...
    if (_.find(memberships, m => m.membership_type.has_max_number_of_classes_per_week)) {
      const mondayInSameWeekAsClass = sails.helpers.util.normalizeDate(classItem.date).startOf('isoWeek');
      const sundayInSameWeekAsClass = sails.helpers.util.normalizeDate(classItem.date).endOf('isoWeek');
    ...
    memberships = _.filter(
      memberships,
      (m) => {
        const membershipPause = _.sortBy(m.membership_pauses, 'start_date').pop();
