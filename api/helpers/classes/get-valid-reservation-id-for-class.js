module.exports = {
  ...
  friendlyName: 'Get valid reservation id for class',

  description: 'Gets all valid reservation id that a specific user has for a class, including or excluding the ones already used for the class.',

  inputs: {
    user: {
      type: 'ref',
      description: 'The user to get valid reservation id for.',
      required: true,
    },

    classItem: {
      type: 'ref',
      description: 'The class to test the validity of the reservation id against.',
      required: true,
    },

    includeAlreadyUsedClassPasses: {
      type: 'boolean',
      description: 'Should the returned list include reservation id that have already been used to sign up for the class?',
      required: true,
    },
  ...
  fn: async (inputs, exits) => {
    ...
    // Filter for classes_left on fixed_count reservation id
    classPasses = _.filter(
      classPasses,
      classPass => {
    ...
    // Filter for unused reservation id
    ...
    // Filter reservation id with max number of simultaneous bookings
