...
describe('create-signup', async function () {
  ...
  after(async () => {
    ...
    await ClassSignup.destroy({id: signupWithMembership.id});
  });
  ...
  it('should throw "userNotFound" if user is not in the database. Test with ID input.', async () => {
    ...
    await assertAsyncThrows(
      async () => {
        await sails.helpers.classes.createSignup.with({
          user: 9999999,
  ...
  it('should throw "userAndClassAreOnDifferentClients" if user and class are on different clients', async () => {

    const classOnDifferentClient = await Class.create({
    ...
    await assertAsyncThrows(
      async () => {
        await sails.helpers.classes.createSignup.with({
          user: fixtures.userAlice,
          classItem: classOnDifferentClient,
        });
      },
  ...
  it('should throw "classIsFull" if there are no available seats and overbooking parameter is false', async () => {
    ...
    await assertAsyncThrows(
      async () => {
        return await sails.helpers.classes.createSignup.with({
  ...
  it('should allow overbooking if overbooking parameter is true', async () => {
    ...
    const newSignup = await sails.helpers.classes.createSignup.with({
      user: fixtures.userBill,
      classItem: yogaClass2,
    ...
    await ClassSignup.destroy({id: [signup1.id, signup2.id, newSignup.id]});
  ...
  it('should find and use a valid membership if one is available.', async () => {
    ...
    await assertAsyncDbObject(
      async () => {
        newSignup = await sails.helpers.classes.createSignup.with({
          user: fixtures.userAlice,
          classItem: yogaClass2,
        });
        return newSignup;
    ...
    await ClassSignup.destroy({id: newSignup.id});
  });


  it('should find and use a valid class pass if one is available. And decrease classes_left', async () => {

    const classPassDanceTenClassesUserBill = await ClassPass.create({
      user: fixtures.userBill.id,
    ...
    await assertAsyncDbObject(
      ...
      {
        used_class_pass: classPassDanceTenClassesUserBill.id,
      },
    );

    const classPassAfterTest = await ClassPass.findOne(classPassDanceTenClassesUserBill.id);
    ...
    await ClassPass.destroy({id: classPassDanceTenClassesUserBill.id});
    await ClassSignup.destroy({id: newSignup.id});
  ...
  it('should only create one signup on two simultaneous requests', async () => {
    ...
    let throwCount = 0;
    await Promise.all([
      sails.helpers.classes.createSignup.with({
    ...
    const createdSignups = await ClassSignup.find({
      user: fixtures.userDennis.id,
      'class': yogaClass1.id,
    });
    ...
    await ClassSignup.destroy({id: _.map(createdSignups, 'id')});
  ...
  it('should only create one signup on three simultaneous requests', async () => {
    ...
    expect(throwCount).to.be.at.most(2);
  ...
  it('should only create one signup on two simultaneous requests if class pass has one class left', async () => {
    ...
    // Depending on the timing inside createSignup, it may throw zero or one time(s)
    expect(throwCount).to.be.at.least(0);
    expect(throwCount).to.be.at.most(1);

    const createdSignupsForClass1 = await ClassSignup.find({
      user: fixtures.userDennis.id,
      class: yogaClass1.id,
    });
    const createdSignupsForClass2 = await ClassSignup.find({
      user: fixtures.userDennis.id,
      class: yogaClass2.id,
    });
    ...
    await ClassSignup.destroy({id: _.map(createdSignupsForClass1, 'id')});
    await ClassPass.destroy({id: classPassDennis.id});
