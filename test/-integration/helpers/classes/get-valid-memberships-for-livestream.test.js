...
describe('get-valid-memberships-for-livestream', async function () {
  ...
  before(async () => {
    ...
    signup2 = await ClassLivestreamSignup.create({
      'class': yogaClass2.id,
      user: fixtures.userBill.id,
      used_membership: membershipYogaBillTwoClassesPerWeek.id,
    }).fetch();

    signup3 = await ClassLivestreamSignup.create({
      'class': yogaClass3.id,
      user: fixtures.userBill.id,
      used_membership: membershipYogaBillTwoClassesPerWeek.id,
    }).fetch();

    signupForCancelledClass = await ClassLivestreamSignup.create({
  ...
  after(async () => {
    ...
    await ClassLivestreamSignup.destroy({id: [signupWithMembership.id, signup2.id, signup3.id, signupForCancelledClass.id]});
  });
  ...
  it('should throw "classNotFound" if class is not in the database. Test with ID input.', async () => {
    ...
    await assertAsyncThrows(
      async () => {
        await sails.helpers.classes.getValidMembershipsForLivestream.with({
  ...
  it('should return an array of memberships valid for the specified class, not including already used memberships.', async () => {
    ...
    let validMemberships = await sails.helpers.classes.getValidMembershipsForLivestream.with({
      user: fixtures.userAlice,
    ...
    assert.deepEqual(
      validMembershipIds,
      [membershipCancelledRunningValid.id],
    );
  ...
  it('should exclude memberships that have reached their limit of classes per week', async () => {
    ...
    const membershipUnlimited = await Membership.create({
      client: testClientId,
      user: fixtures.userDennis.id,
      membership_type: fixtures.membershipTypeYogaUnlimitedLivestream.id,
      status: 'active',
    ...
    assert.deepStrictEqual(
      ...
      [membershipUnlimited.id],
    );

    await Class.destroy({id: _.map(_.concat([classInQuestion], classesToCountTowardsLimit), 'id')});
    await Membership.destroy({id: [membership.id, membershipUnlimited.id]});
